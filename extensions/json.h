...
template <typename Stream>
void render_json_value(Stream &ss, const QString &s) {
    render_json_value(ss,s.toStdString());
}
...
inline void read_json(reader_t &rd, QString &val, bool unorder = false) {
    auto stdStr = val.toStdString();
   read_json(rd,stdStr,unorder);
   val = QString::fromStdString(stdStr);
}
